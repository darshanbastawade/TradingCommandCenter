...
public static class WalkForwardCommands
{
    ...
    public static bool IsCommand(string[] args) => args.Length > 0 && args[0] == "run-walk-forward";
        ...
        TextWriter error, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var result = TimeSeriesValidationEngine.RunWalkForward(
                ...
                new(trainingSessions, testingSessions, embargoSessions, anchored), backtestSettings);
            var artifact = WalkForwardTestArtifactBuilder.Build(result, candles, zone, anchored);
            await WriteNewAtomicallyAsync(destination, BacktestReportJson.Serialize(artifact), cancellationToken);

            await output.WriteLineAsync(JsonSerializer.Serialize(new
            {
                status = "walk-forward-report-created",
                output = destination,
                strategyId = artifact.Folds.Select(fold => fold.SelectedStrategyId).Distinct().Single(),
                trainingMode = artifact.TrainingMode,
                folds = artifact.FoldCount,
                unusedTrailingSessions = artifact.UnusedTrailingSessionCount,
                profitableFolds = artifact.ProfitableFoldCount,
                profitableFoldRate = artifact.ProfitableFoldRate,
                trades = artifact.CombinedReport.Summary.TotalTrades,
                netPnl = artifact.CombinedReport.Summary.NetPnl
            }));
            return 0;
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            await error.WriteLineAsync("Walk-forward test cancelled; no completed report should be assumed.");
            return 130;
        }
        ...
        {
            await error.WriteLineAsync("Unable to create the walk-forward report. The output must be a new writable .json file.");
            return 2;
        }
        catch (Exception)
        {
            await error.WriteLineAsync("Walk-forward testing failed. Check database readiness, the requested range and report settings.");
            return 3;
        }
    }
    ...
    private static Dictionary<string, string> Parse(string[] args)
    {
        ...
        {
            "instrument-id", "timeframe", "from", "to", "training-sessions", "testing-sessions",
            "embargo-sessions", "training-mode", "initial-capital", "allowed-risk", "maximum-capital",
            "maximum-lots", "slippage-bps", "cost-profile", "strategy-id", "output"
        };
